...
export async function upsertWorkScheduleAction(formData: FormData): Promise<ScheduleActionResult> {
  ...
  try {
    ...
    if (!technicianUid) throw new Error(t.actionErrors.schedule_technicianUidRequired);

    const workDaysRaw = formData.getAll("workDays").map(Number).filter((d) => d >= 0 && d <= 6);
    if (workDaysRaw.length === 0) throw new Error(t.actionErrors.schedule_workDayRequired);
    ...
    if (!Number.isFinite(expectedDurationMinutes)) throw new Error(t.actionErrors.schedule_durationInvalid);
    if (hourlyRate !== undefined && !Number.isFinite(hourlyRate)) throw new Error(t.actionErrors.schedule_hourlyRateInvalid);
    ...
    return { error: error instanceof Error ? error.message : t.actionErrors.schedule_saveFailed };
  }
}
